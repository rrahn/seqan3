...
struct iterator_fixture<matrix_iterator_t> : public ::testing::Test
{
    ...
    using materialised_column_t = std::vector<cell_t, seqan3::aligned_allocator<cell_t, alignof(cell_t)>>;

    using iterator_tag = std::input_iterator_tag;
    static constexpr bool const_iterable = false;

    // Single column with 5 entries as the seq2 has size 4 (need one more for the initialisation row).
    materialised_column_t column = materialised_column_t{5, std::tuple{score_t{0}, score_t{0}, score_t{0}}};
