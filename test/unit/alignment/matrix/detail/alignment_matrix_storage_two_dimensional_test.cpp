...
struct alignment_matrix_storage_two_dimensional_test : public ::testing::Test
{
...
TEST_F(alignment_matrix_storage_two_dimensional_test, matrix_storage_concept)
{
    EXPECT_TRUE(seqan3::detail::alignment_matrix_storage<storage_t>);
}

TEST_F(alignment_matrix_storage_two_dimensional_test, cols)
{
    s.resize(rdim, cdim);
    EXPECT_EQ(s.cols(), cdim.get());
}

TEST_F(alignment_matrix_storage_two_dimensional_test, rows)
{
    s.resize(rdim, cdim);
    EXPECT_EQ(s.rows(), rdim.get());
}

TEST_F(alignment_matrix_storage_two_dimensional_test, column_at)
{
...
TEST_F(alignment_matrix_storage_two_dimensional_test, clear)
{
    s.resize(rdim, cdim);
    s.clear();
    EXPECT_EQ(s.cols(), 0U);
