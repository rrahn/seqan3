...
class affine_gap_policy_mock :
    public seqan3::detail::affine_gap_banded_policy<affine_gap_policy_mock<cell_type>, cell_type>,
    public check_score_mock
{
public:
    using base_t = seqan3::detail::affine_gap_banded_policy<affine_gap_policy_mock<cell_type>, cell_type>;
...
template <typename t>
struct affine_gap_policy_test : ::testing::Test
{
    using cell_t = std::tuple<t, t, seqan3::detail::ignore_t>;
...
TYPED_TEST_CASE(affine_gap_policy_test, test_types);

TEST(affine_gap_banded_policy, construction)
{
    EXPECT_TRUE((std::is_default_constructible_v<affine_gap_policy_mock<std::tuple<int, int, seqan3::detail::ignore_t>>>));
    EXPECT_TRUE((std::is_copy_constructible_v<affine_gap_policy_mock<std::tuple<int, int, seqan3::detail::ignore_t>>>));
    EXPECT_TRUE((std::is_move_constructible_v<affine_gap_policy_mock<std::tuple<int, int, seqan3::detail::ignore_t>>>));
    EXPECT_TRUE((std::is_copy_assignable_v<affine_gap_policy_mock<std::tuple<int, int, seqan3::detail::ignore_t>>>));
    EXPECT_TRUE((std::is_move_assignable_v<affine_gap_policy_mock<std::tuple<int, int, seqan3::detail::ignore_t>>>));
    EXPECT_TRUE((std::is_destructible_v<affine_gap_policy_mock<std::tuple<int, int, seqan3::detail::ignore_t>>>));
}

TYPED_TEST(affine_gap_banded_policy, make_cache)
{

    affine_gap_policy_mock<typename TestFixture::cell_t> mock{};

    // We need different schemes here.
...
TEST(affine_gap_banded_policy, compute_first_band_cell)
{
    ...
    affine_gap_policy_mock<std::tuple<int, int, seqan3::detail::ignore_t>> mock{};
