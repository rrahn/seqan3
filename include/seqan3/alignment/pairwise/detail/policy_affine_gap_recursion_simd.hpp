...
protected:
    ...
    explicit policy_affine_gap_recursion_simd(alignment_configuration_t const & config)
    {
        ...
        auto && tmp_scheme = config.template value_or<align_cfg::gap>(gap_scheme{gap_score{-1}, seqan3::gap_open_score{-10}});

        gap_extension_score = simd::fill<score_type>(tmp_scheme.get_gap_score());
        gap_open_score = simd::fill<score_type>(tmp_scheme.get_gap_open_score()) + gap_extension_score;
    }
    //!\}
};
} // namespace seqan3::detail
