...
protected:
    ...
    void compute_matrix(sequence1_t & sequence1, sequence2_t & sequence2)
    {
        ...
        thread_local coordinate_matrix<matrix_index_t> local_coordinate_matrix{};

        local_score_matrix.reset_matrix(sequence1, sequence2);
        local_coordinate_matrix.reset_matrix(
                column_index_type{std::ranges::distance(sequence1) + 1},
                row_index_type{std::ranges::distance(sequence2) + 1});
        ...
        auto && [final_column, coordinate_column] = *matrix_iter;
        auto final_column_iter = final_column.begin();
        auto coordinate_iter = coordinate_column.begin();
        this->track_last_column_cell(*final_column_iter, *coordinate_iter);

        for ([[maybe_unused]] auto const & unused : sequence2)
            this->track_last_column_cell(*++final_column_iter, *++coordinate_iter);

        this->track_final_cell(*final_column_iter, *coordinate_iter);
    }
