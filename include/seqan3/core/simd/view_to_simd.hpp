...
public:
    ...
    constexpr iterator_type(view_to_simd & this_view) : this_view{&this_view}, current_chunk_pos{0}
    {
        ...
        // The batch is not empty but might not be full either. So the empty slots are initialised with the
        // iterator of the first sequence set to the end and the end of the first sequence.
        if (seq_id > 0)
        {
